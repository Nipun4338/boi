...
foreach ($data as $row) {
 ...
 <div class="card " style="margin: 10px;font-family:Helvetica">
  ...
  <div class="card-body">' .
        $row["comment"] .
        '</div>
  <div class="card-footer text-muted" align="right"><button type="button" class="btn btn-primary reply btn-sm" id="' .
        $row["comment_id"] .
        '">Reply</button></div>
 </div>
 ';
...
function get_reply_comment($connection, $parent_id = 0, $marginleft = 0)
{
 ...
 ";
    $output = "";
    ($result1 = mysqli_query($connection, $sql)) or
   ...
   <div class="card" style="margin-left:' .
                $marginleft .
                'px;margin-top: 10px;margin-bottom: 10px;font-family:Helvetica">
    <div class="card-header">By <b>' .
                $row["sender_name"] .
                "</b> on <i>" .
                date("M j, Y g:i A", strtotime($row["date"])) .
