...
if (isset($_POST["registerbtnbook"])) {
    ...
    $query = sprintf(
        "INSERT INTO books (name,author,owner_id,category,price,present_condition,location,status,created_date,updated_date)
...
if (isset($_POST["updatebtnbook"])) {
    ...
    $datetime = date("Y-m-d H:i:s");
    $query = sprintf(
        "update books set price='$price',present_condition='%s', updated_date='$datetime' where book_id='$id'",
        mysqli_real_escape_string($connection, $details),
    );
    $query_run = mysqli_query($connection, $query);
    if ($query_run) {
